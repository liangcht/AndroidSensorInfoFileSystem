...
int syscall_only_write(struct sensor_information *si)
{
	...
	snprintf(temp, sizeof(temp), "TimeStamp:%ld,Centilux:%d\n",
		 curr_time, si->centilux);
	add_to_buf(lumi_sde, temp);
	memset(temp, 0, 500);

	snprintf(temp, sizeof(temp), "TimeStamp:%ld,Centiproximity:%d\n",
...
static struct dentry *sensorfs_lookup_de(
	struct sensorfs_dir_entry *de,
	struct inode *dir,
